...
static StreamBufferHandle_t LedStreamBuffHa_t = NULL;
...
void Bled_task(void* arg)
{
	...
	while(1)
	{
		...
		sprintf(tmpbuff,"t:%d abcdefghigk",cnt++);
		//向流缓冲发送数据
		re = xStreamBufferSend(LedStreamBuffHa_t,tmpbuff,sizeof(tmpbuff),portMAX_DELAY);
		

		vTaskDelay(1000);

		xSemaphoreTake(xOled_Semaphore,portMAX_DELAY);
		sprintf(oled_buff,"send:%d ",re);
...
{
	uint8_t mycount,re = 0;		//控制显示的行
	char receive_buff[100] = {0};
	while(1)
	{
//		//接收流缓冲的数据
//		re = xStreamBufferReceive(LedStreamBuffHa_t , receive_buff, sizeof(receive_buff),portMAX_DELAY);
		//查询流缓冲的数据个数
		re =  xStreamBufferBytesAvailable(LedStreamBuffHa_t);

		
		xSemaphoreTake(xOled_Semaphore,portMAX_DELAY);
		
		sprintf(oled_buff,"%d  %d",mycount++,re);
		OLED_ShowString(0,4,(uint8_t*)oled_buff,16);
...
void start_task(void* arg)
{
	...
	LedStreamBuffHa_t = xStreamBufferCreate(40,1);
