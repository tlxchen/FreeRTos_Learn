...
/*Rled的·一个消息缓冲*/
static MessageBufferHandle_t  LedMessageBuffHa_t = NULL;
...
void Bled_task(void* arg)
{
	...
	while(1)
	{
		...
		//向消息缓冲发送数据
		 xMessageBufferSend( LedMessageBuffHa_t ,tmpbuff,sizeof(tmpbuff) , portMAX_DELAY);
...
{
	uint8_t mycount;		//控制显示的行
	char receive_buff[100] = {0};
	while(1)
	{
		//接收消息缓冲的数据
		xMessageBufferReceive(LedMessageBuffHa_t,receive_buff,sizeof(receive_buff),portMAX_DELAY);

		
		xSemaphoreTake(xOled_Semaphore,portMAX_DELAY);
		
		sprintf(oled_buff,"M%d %s",mycount++,receive_buff);
		OLED_ShowString(0,4,(uint8_t*)oled_buff,16);
...
void start_task(void* arg)
{
	...
	LedMessageBuffHa_t = xMessageBufferCreate(24);
